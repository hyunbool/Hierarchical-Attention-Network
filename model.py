...
class HierarchialAttentionNetwork(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, segment_rnn_size, word_rnn_layers,
                 sentence_rnn_layers, segment_rnn_layers, word_att_size, sentence_att_size, segment_att_size, dropout=0.5):
        """
        ...
        self.segment_attention = SegmentAttention(vocab_size, emb_size, word_rnn_size, sentence_rnn_size, segment_rnn_size, word_rnn_layers,
                 sentence_rnn_layers, segment_rnn_layers, word_att_size, sentence_att_size, segment_att_size, dropout)
    ...
    def forward(self, documents, segments_per_document, sentences_per_segment, words_per_sentence):
        """
        Forward propagation.
        ...
        document_embeddings, word_alphas, sentence_alphas, segment_alphas = self.segment_attention(documents, segments_per_document, sentences_per_segment, words_per_sentence)
        # Classify
        scores = self.fc(self.dropout(document_embeddings))  # (n_documents, n_classes)

        return scores, word_alphas, sentence_alphas, segment_alphas
...
class SegmentAttention(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, segment_rnn_size, word_rnn_layers, sentence_rnn_layers,
                 segment_rnn_layers, word_att_size, sentence_att_size, segment_att_size, dropout):
        """
        :param vocab_size: number of words in the vocabulary of the model
        :param emb_size: size of word embeddings
        :param word_rnn_size: size of (bidirectional) word-level RNN
        ...
        self.sentence_attention = SentenceAttention(vocab_size, emb_size, word_rnn_size, sentence_rnn_size, word_rnn_layers,
                 sentence_rnn_layers, word_att_size, sentence_att_size, dropout)

        # Bidirectional sentence-level RNN
        self.segment_rnn = nn.GRU(2 * sentence_rnn_size, segment_rnn_size, num_layers=segment_rnn_layers,
                                   bidirectional=True, dropout=dropout, batch_first=True)

        # Sentence-level attention network
        self.segment_attention = nn.Linear(2 * segment_rnn_size, segment_att_size)

        # Sentence context vector to take dot-product with
        self.segment_context_vector = nn.Linear(segment_att_size, 1,
    ...
    def forward(self, documents, segments_per_document, sentences_per_segment, words_per_sentence):
        ...
        segments, sentence_alphas = self.sentence_attention(packed_segments.data,
                                                     packed_sents_per_segments.data,
                                                            words_per_sentence)  # (n_sentences, 2 * word_rnn_size), (n_sentences, max(words_per_sentence))
        segments = self.dropout(segments)

        # Apply the sentence-level RNN over the sentence embeddings (PyTorch automatically applies it on the PackedSequence)
        packed_sentences, _ = self.segment_rnn(PackedSequence(data=segments,
                                                               batch_sizes=packed_segments.batch_sizes,
                                                               sorted_indices=packed_segments.sorted_indices,
                                                               unsorted_indices=packed_segments.unsorted_indices))  # a PackedSequence object, where 'data' is the output of the RNN (n_sentences, 2 * sentence_rnn_size)

        # Find attention vectors by applying the attention linear layer on the output of the RNN
        att_s = self.segment_attention(packed_segments.data)  # (n_sentences, att_size)
        att_s = torch.tanh(att_s)  # (n_sentences, att_size)
        # Take the dot-product of the attention vectors with the context vector (i.e. parameter of linear layer)
        att_s = self.segment_context_vector(att_s).squeeze(1)  # (n_sentences)
...
class SentenceAttention(nn.Module):
    ...
    def forward(self, segments, sentences_per_segment, words_per_sentence):
        """
        Forward propagation.
        ...
        packed_sentences = pack_padded_sequence(segments,
                                                lengths=sentences_per_segment.tolist(),
                                                batch_first=True,
